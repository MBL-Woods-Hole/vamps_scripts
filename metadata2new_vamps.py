...
DATASETS = {}
PROJECTID = 0
required_metadata_fields = [ "altitude", "assigned_from_geo", "collection_date", "depth", "country", "elevation", "envo_biome","env_biome", "envo_feature","env_feature", "envo_material","env_matter", "latitude", "longitude", "public","taxon_id","description","common_name"];
...
def read_vamps_metadata(args):
    ...
    if args.delim == 'comma':
        lines = list(csv.reader(open(args.infile, 'rb'), delimiter=','))
    else:
        lines = list(csv.reader(open(args.infile, 'rb'), delimiter='\t'))
    dataset_lookup = {}
    TMP_METADATA_ITEMS = {}
