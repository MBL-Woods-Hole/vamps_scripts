...
# METADATA
mysql -B -h $SITE vamps -e "SELECT * FROM vamps_submissions where temp_project = '$P';" | sed "s/'/\'/;s/\t/\",\"/g;s/^/\"/;s/$/\"/;s/\n//g" > project_$P.csv
mysql -B -h $SITE vamps -e "SELECT * FROM vamps_submissions_tubes where project_name = '$P';" | sed "s/'/\'/;s/\t/\",\"/g;s/^/\"/;s/$/\"/;s/\n//g" > metadata_$P.csv
...
# USER
mysql -B -h $SITE vamps -e "SELECT id,concat(first_name,' ',last_name) as contact,user as username, email, institution, first_name, last_name, active, security_level from vamps_auth;" | sed "s/'/\'/;s/\t/\",\"/g;s/^/\"/;s/$/\"/;s/\n//g" >> user_contact_$P.csv
