...
def backup_old_group_file():
    strg = str(datetime.datetime.now().strftime('%Y%m%d%H%M%S'))
    old_group_file = os.path.join(args.json_file_path,'vamps2--metadata.json')
    bkup_group_file = os.path.join(args.json_file_path,'vamps2--metadata'+strg+'.json')
    print('Re-Naming ',old_group_file,'to',bkup_group_file)
    os.rename(old_group_file, bkup_group_file)
    
    
def clean_group_file():
    ...
    with open(group_file) as f:
        ...
        for did in data:
            ...
            if args.field_name in data[did]:
                print('removing did:',did,' field:',args.field_name,' val:',data[did][args.field_name])
                deleted_dids.append(did)
            else:
                new_data[did] = data[did]
    
    new_group_file = os.path.join(args.json_file_path,'vamps2--metadataNEW.json')
    if deleted_dids:
        with open(new_group_file, 'w') as ofile:
            json.dump(new_data, ofile)
        print('Wrote new: ',new_group_file)
        print("Deleted dids",deleted_dids)
        backup_old_group_file()
        print('Re-Naming ',new_group_file,'to',group_file)
        os.rename(new_group_file, group_file)
        os.chmod(group_file, 0o777)
    else:
        print('Not creating new file - nothing deleted')
    
    
    
    files_deleted = 0
    for did in deleted_dids:
        file_silva  = os.path.join(args.json_file_path,'vamps2--datasets_silva119',did+'.json')
        file_rdp    = os.path.join(args.json_file_path,'vamps2--datasets_rdp_2.6',did+'.json')
        file_generic= os.path.join(args.json_file_path,'vamps2--datasets_generic',did+'.json')
        if os.path.exists(file_silva):
            os.remove(file_silva)
            print('Deleted Silva',did)
            files_deleted += 1
        if os.path.exists(file_rdp):
            os.remove(file_rdp)
            print('Deleted RDP',did)
            files_deleted += 1
        if os.path.exists(file_generic):
            os.remove(file_generic)
            print('Deleted Generic',did)
            files_deleted += 1
...
if __name__ == '__main__':
    ...
    metadata_files_synchrony.py
        ...
        -s/--search_where    choices=['group_file', 'single_files', 'database']
        -f/--field_name     to search for REQUIRED
    ...
    parser.add_argument("-v", "--verbose",
                ...
                help="")
    parser.add_argument("-s", "--search_where", choices=['group_file', 'single_files', 'database'],
                required=True,  action='store',  dest = "clean_where",  
                help="group_file single_files, database")
    parser.add_argument("-f", "--field_name",
                required=True,  action='store',  dest = "field_name",  default=False,
    ...
    if args.clean_where not in searches:
        sys.exit()
    print('DATABASE:',NODE_DATABASE)
    ask = input("\nAre you sure you want to remove all datasets with this field `"+args.field_name+"` from the "+args.clean_where+" on "+args.dbhost+": ")
    if ask.capitalize() == 'Y': 
        if args.clean_where == 'group_file':
            clean_group_file()
        elif args.clean_where == 'single_files':
            print("single_files not implemented yet")
        else:
            print("database clean not implemented yet")
        #go_list(args)
    else:
