...
def parse_matrix_file():
    ...
    for row in reader:
        ...
        if not row:
            continue
        if n==0 and row[0] == 'TaxByTax':
            print ('Found TaxByTax')
            
        elif n==1:   # dataset row
            
            dsets = row[:-2]
            datasets = [n.split('--')[-1] for n in dsets]
            tax_string_index = len(row)-1
            if args.verbose:
                print('tax_string_index',tax_string_index)
            ...
            raw_tax_string = row[tax_string_index]
            # cleanup
    ...
    myusage = """usage: vamps_script_database_loader.py  [options]
       ...
       BEST::TaxByTax 
         where
            ...
            -p/--project        project
            -u/--user           vamps user
            -s/--source_file_type    Default: tax_end (TaxByTax) others (untested) tax1, tax2, tax_end, slp, none
            -v/--v
            
            Matrix Pipeline:  Database: matrix_taxonomy_info; Files: datasets--generic/
            1- download TaxByTax file from old VAMPS to use for input file
                for matrix_loader.py 
                />head test.csv 
                TaxByTax
                PGQ_FG_1--1	PGQ_FG_1--10	PGQ_FG_1--11    Rank	Taxonomy
                1	0	0	species	Fungi;Ascomycota;Arthoniomycetes;Arthoniales;Arthoniaceae;Arthothelium;Arthothelium_spectabile
                0	6	2	species	Fungi;Ascomycota;Ascomycota;Ascomycota;Ascomycota;Acremonium;Acremonium_psammosporum
                2	7	5	species	Fungi;Ascomycota;Ascomycota;Ascomycota;Ascomycota;Acremonium;Acremonium_sp.
            2- ./matrix_loader.py -host vamps -i filename.csv -p project_name
            3- ./assign_default_metadata.py -host vamps -pid 4449
            4- ./rebuild_vamps_files.py -host vamps -pids 3631 -units matrix
...
"""
    ...
    parser.add_argument("-u", "--user",    
                required=False,  action="store",   dest = "user", default='admin',
                help = 'vamps user name')
    parser.add_argument('-s', '--source_file_type',         
                required=False,   action="store",   dest = "source", default='tax_end',  # tax1, tax2, tax_end, slp, none
                help = '')
